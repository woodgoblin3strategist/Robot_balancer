...
#define range_time_for_filter 0.75
...
void setMotors(int speed) {
  //
  servo_angle = constrain(speed, 0, 180);
  // чтобы устранить люфты (при заданных значениях угла серва не поворачивается, поэтому меняем их на ближайшие движемые)
  if((servo_angle>85) && (servo_angle<90)){
    servo_angle = 85;
  }
  else if((servo_angle>90) && (servo_angle<95)){
    servo_angle = 96;
  }
...
void loop() {
  ...
  if (Tnext <= millis()) {
    ...
    // угол покоя минусуем потому что поставил датчик mpu6050 на другую сторону
    servo_speed = stillAngle - (Kp * (error) + Ki * (errorSum)*sampleTime - Kd * (currentAngle - prevAngle) / sampleTime);
